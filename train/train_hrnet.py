...
from lib.network.rtpose_hrnet import get_model
...
DATA_DIR = '/home/mgy/detectron2/datasets/coco'
...
def train_cli(parser):
    ...
    group.add_argument('--n-images', default=8000, type=int,
                       help='number of images to sample')
    group.add_argument('--duplicate-data', default=None, type=int,
                       help='duplicate data')
    group.add_argument('--loader-workers', default=12, type=int,
...
def train_factory(args, preprocess, target_transforms):
    train_datas = [datasets.CocoKeypoints(
        ...
        input_x=cfg.DATASET.IMAGE_SIZE,
        input_y=cfg.DATASET.IMAGE_SIZE,
        stride=cfg.MODEL.DOWNSAMPLE
    ) for item in args.train_annotations]
    train_data = torch.utils.data.ConcatDataset(train_datas)

    # Add
    # train_size = int(len(train_datas[0]) * 0.7)
    # val_size = len(train_datas[0]) - train_size
...
model_save_filename = './network/weight/my_hrnet.pth'
